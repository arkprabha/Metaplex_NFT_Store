...
fn create_auction(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    ...
    let floor = app_matches.value_of("minimum").map(|price| {
        ...
        if app_matches.is_present("blind") {
            let hash = hash::hashv(&[&price.to_be_bytes(), &salt.to_be_bytes()]);
            PriceFloor::BlindedPrice(hash)
        } else {
            PriceFloor::MinimumPrice([price, 0, 0, 0])
        }
    });
...
fn inspect_auction(app_matches: &ArgMatches, _payer: Keypair, client: RpcClient) {
    ...
    let auction: spl_auction::processor::AuctionData =
        try_from_slice_unchecked(&auction_data.data).unwrap();
    ...
    ",
        ...
        match auction.price_floor {
            PriceFloor::None(_) => "No Floor".to_string(),
            PriceFloor::MinimumPrice(min) => format!("Minimum Bid: {}", min[0]),
            PriceFloor::BlindedPrice(_) => "Minimum Price Concealed".to_string(),
        },
    );

    if let BidState::EnglishAuction { ref bids, max } = auction.bid_state {
...
fn place_bid(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    use spl_auction::{instruction, processor::PlaceBidArgs, PREFIX};
    ...
    let mut transaction = Transaction::new_with_payer(&instructions, Some(&payer.pubkey()));
    let recent_blockhash = client.get_recent_blockhash().unwrap().0;
    let signers = vec![&payer, &bidder, &bid_pot, &transfer_authority];
...
fn claim_bid(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    use spl_auction::{instruction, processor::ClaimBidArgs, PREFIX};
    ...
    let bidpot: spl_auction::processor::BidderPot =
        try_from_slice_unchecked(&bidpot_data.data).unwrap();
    ...
    let mut transaction = Transaction::new_with_payer(&instructions, Some(&payer.pubkey()));
    let recent_blockhash = client.get_recent_blockhash().unwrap().0;
    let signers = vec![&payer, &destination];

    // Doesn't matter if this transaction fails.
    transaction.sign(&signers, recent_blockhash);
    client.send_and_confirm_transaction(&transaction).unwrap();

    let instructions = [instruction::claim_bid_instruction(
        program_key,
        payer.pubkey(),
        destination.pubkey(),
        bidder.pubkey(),
...
fn cancel_bid(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    use spl_auction::{instruction, processor::CancelBidArgs, PREFIX};
...
fn end_auction(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    use spl_auction::{instruction, processor::EndAuctionArgs};
    ...
    let revealer = if let Some(salt) = app_matches.value_of("salt") {
        ...
        let hash = hash::hashv(&[&salt.to_be_bytes(), &price.to_be_bytes()]);
        println!("Revealing Hash: {}", hash);
        let hash = hash::hashv(&[&price.to_le_bytes(), &salt.to_le_bytes()]);
        println!("Revealing Hash: {}", hash);
        let hash = hash::hashv(&[&salt.to_le_bytes(), &price.to_le_bytes()]);
        println!("Revealing Hash: {}", hash);
        Some((price, salt))
    } else {
        None
...
fn start_auction(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    use spl_auction::{instruction, processor::StartAuctionArgs};
